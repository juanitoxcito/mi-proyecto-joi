...
# Asegúrate de que la ruta a firebase_credentials.json sea correcta para tu entorno de compilación
# En el entorno de GitHub Actions, este archivo estará en la raíz del repositorio.
# Si lo estás probando localmente, asegúrate de que la ruta sea absoluta o relativa a donde ejecutas el script.
# Para la compilación en GitHub Actions, la ruta 'firebase_credentials.json' (directamente en la raíz) es la correcta.
...
FIRESTORE_DOC_ID = 'juan_facts' # Usamos un ID fijo para Juan
...
class JoiApp(App):
    ...
    def _start_speech_recognition(self):
        try:
            ...
            # El idioma es 'es-ES' para español de España, puedes cambiarlo si necesitas otro dialecto.
            # El resultado será una lista de posibles transcripciones.
            results = stt.listen(language='es-ES', show_partial=False)

            # Si hay resultados, tomamos la primera (la más probable)
            if results and len(results) > 0:
                recognized_text = results[0]
                # Programamos la actualización de la UI en el hilo principal de Kivy
            ...
            # Manejo de errores si el reconocimiento de voz falla
            error_message = f"Error en el reconocimiento de voz: {e}"
            # Algunos errores comunes:
            # - No hay conexión a internet (si el motor de STT lo requiere)
            # - Permiso de micrófono no concedido (importante en Android)
            # - Motor de reconocimiento de voz no disponible en el dispositivo
            Clock.schedule_once(lambda dt: self._update_chat_display(error_message), 0)
        finally:
            # Siempre re-habilitamos el botón al finalizar
            Clock.schedule_once(lambda dt: self._reset_listen_button(), 0)

    def _process_recognized_text(self, user_input):
        # Esta función se llama después de reconocer la voz
        # Muestra el texto reconocido y luego obtiene la respuesta de Joi
        self.chat_display.text = f"Tú: {user_input}\nJoi: Pensando..."
        self.chat_display.cursor = (0, len(self.chat_display.text.splitlines()))

        # Inicia la generación de respuesta de Joi en un hilo separado
    ...
    def _get_joi_response_threaded(self, user_input):
        # Esta es la función que llama a Gemini y procesa la respuesta
        ...
        try:
            ...
            # --- Lógica para procesar la respuesta de Joi (Copiado de primer_hola.py) ---
    ...
    def _update_chat_display(self, text):
        ...
        self.chat_display.text = f"Tú: {text}\nJoi: (Respuesta de Joi aquí...)" # Aquí se mostrará la respuesta de Joi
        self.chat_display.cursor = (0, len(self.chat_display.text.splitlines())) # Desplazar al final

    def _update_chat_display_with_joi_response(self, user_input, joi_response):
        # Esta función actualiza el TextInput con la conversación completa
        current_text = self.chat_display.text.replace("Joi: Pensando...", "") # Elimina el "Pensando..."
