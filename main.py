...
jobs:
  build:
    runs-on: ubuntu-latest # Ejecutamos en un runner estándar de Ubuntu

    env: # Establecemos HOME para todo el trabajo para evitar problemas de permisos de checkout
      HOME: /tmp
    ...
    # Paso AÑADIDO: Este paso utiliza los secretos que configuraste para iniciar sesión en Docker Hub
    # Esto soluciona el error 'toomanyrequests'
    ...
    - name: Build Android APK using Docker Container # Todo en un solo comando docker run
      run: |
        ...
        docker run --rm \
          ...
          bash -c " \
            ...
            pip install --upgrade pip && \
            pip install buildozer && \
            # ¡CLAVE! Se añaden las nuevas dependencias para el grafo y el LLM
            pip install pyjnius==1.4.2 plyer cython==0.29.36 networkx google-generativeai && \
            rm -rf .buildozer && \
            ...
            # ¡CLAVE! Se añaden los nuevos requisitos al archivo buildozer.spec
            sed -i 's/^#*requirements = .*$/requirements = python3,kivy,kivymd,plyer,cython,networkx,google-generativeai/' buildozer.spec && \
            ...
            sed -i '/^#*android.permissions =/c\android.permissions = VIBRATE,INTERNET,RECORD_AUDIO' buildozer.spec && \
            python3 -m pip install cython==0.29.36 && \
            yes | buildozer android debug --ignore-root && \
